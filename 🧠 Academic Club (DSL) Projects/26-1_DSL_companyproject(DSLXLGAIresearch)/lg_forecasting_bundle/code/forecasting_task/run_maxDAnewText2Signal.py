...
def run_epoch(loader, model, args, device, optimizer=None):
    ...
    with torch.enable_grad() if is_train else torch.no_grad():
        for batch in loader:
            ...
            # Directional Penalty on the final step's close price (with clamp to prevent explosion)
            pred_diff = out[:, -1, 3] - px[:, -1, 3]
            true_diff = gt[:, -1, 3] - px[:, -1, 3]
            dir_penalty = torch.clamp(torch.relu(-1.0 * pred_diff * true_diff), max=1.0).mean()
            
            # NLL Loss (Uncertainty-aware MSE)
            # log_var applies to the 4 features
            nll_loss = 0.5 * torch.mean(torch.exp(-log_var) * ((out - gt)**2) + log_var)
            
            if is_train:
                loss = nll_loss + args.dir_weight * dir_penalty
                if args.joint:
                    target = (gt[:, -1, 3] > px[:, -1, 3]).float().unsqueeze(1)
