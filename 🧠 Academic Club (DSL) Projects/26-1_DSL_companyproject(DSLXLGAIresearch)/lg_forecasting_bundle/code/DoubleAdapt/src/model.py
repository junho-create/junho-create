...
class IncrementalManager:
    ...
    def __init__(
        ...
        **kwargs
        ...
        # If True, the "online" phase (phase=="online", i.e. inference() on the eval-period
        # rolling tasks) does a plain frozen forward pass -- no gradient step, no local few-shot
        # adaptation. Needed when the eval-period data comes from a file we're not allowed to
        # train on (e.g. a competition's test.parquet): only the already-train.parquet-fitted
        # weights are used, never updated using eval-period data.
        self.freeze_online = freeze_online
    ...
    def _init_framework(self, model: nn.Module, x_dim: int = None, lr_model=0.001,
                        weight_decay=0.0, need_permute=False, aux_weight=0.0, dir_scale=5.0,
                        label_mean=0.0, label_std=1.0, **kwargs):
    ...
    def _run_task(self, meta_input: Dict[str, Union[pd.Index, torch.Tensor]], phase: str):
        ...
        if self.freeze_online and phase == "online":
            with torch.no_grad():
        ...
        w_train = meta_input.get("w_train")
        if w_train is not None:
            w_train = w_train.to(self.framework.device)
        loss = self.framework.criterion(y_hat, meta_input["y_train"].to(self.framework.device), weight=w_train)
        loss.backward()
...
class DoubleAdaptManager(IncrementalManager):
    ...
    def __init__(
        ...
        super(DoubleAdaptManager, self).__init__(model, x_dim=x_dim, lr_model=lr_model, lr_ma=lr_ma, lr_da=lr_da,
                                                 ...
                                                 aux_weight=aux_weight, dir_scale=dir_scale,
    ...
    def _init_framework(self, model: nn.Module, x_dim=None, lr_model=0.001, need_permute=False,
                        num_head=8, temperature=10, factor_num=6, lr_ma=None, weight_decay=0,
                        aux_weight=0.0, dir_scale=5.0, **kwargs):
        return DoubleAdapt(
    ...
    def _run_task(self, meta_input: Dict[str, Union[pd.Index, torch.Tensor]], phase: str):
        ...
        if self.freeze_online and phase == "online":
            with torch.no_grad():
                if "X_extra" in meta_input and meta_input["X_extra"].shape[0] > 0:
                    X_test = torch.cat([meta_input["X_extra"].to(self.framework.device),
                                        meta_input["X_test"].to(self.framework.device)], 0)
                else:
                    X_test = meta_input["X_test"].to(self.framework.device)
                pred, _ = self.framework(X_test, model=self.framework.model, transform=self.adapt_x)
                if self.adapt_y:
        ...
        loss2 = self.framework.criterion(y_hat, y, weight=w_train)
        diffopt.step(loss2)
        ...
        if phase != "train" and "X_extra" in meta_input and meta_input["X_extra"].shape[0] > 0:
            ...
            if "w_extra" in meta_input:
                w_test = torch.cat([meta_input["w_extra"].to(self.framework.device), meta_input["w_test"].to(self.framework.device), ], 0, )
            else:
                w_test = None
        else:
            X_test = meta_input["X_test"].to(self.framework.device)
            y_test = meta_input["y_test"].to(self.framework.device)
            w_test = meta_input.get("w_test")
            if w_test is not None:
                w_test = w_test.to(self.framework.device)
        ...
        loss = self.framework.criterion(pred, y_test, weight=w_test)
        if self.adapt_y:
            ...
            if self.first_order:
                ...
                with torch.no_grad():
                    ...
                    loss_old = self.framework.criterion(pred2.view_as(y_test), y_test, weight=w_test)
