...
def get_task_data(segments: dict, dataframe) -> Dict[str, Union[np.ndarray, pd.Index]]:
    ...
    has_weight = "weight" in dataframe.columns.get_level_values(0)
    if train_exist:
    ...
    if has_weight:
        processed_meta_input["w_test"] = d_test["weight"].iloc[:, 0]
    if train_exist:
        ...
        if has_weight:
            processed_meta_input["w_train"] = d_train["weight"].iloc[:, 0]
    if extra_exist:
        ...
        if has_weight:
            processed_meta_input["w_extra"] = d_extra["weight"].iloc[:, 0]
...
def preprocess(task_data_list: List[Dict[str, Union[np.ndarray, pd.Index]]],
    ...
    for i, task_data in enumerate(task_data_list):
        ...
        for k in task_data.keys():
            if k.startswith("X") or k.startswith("y") or k.startswith("w"):
