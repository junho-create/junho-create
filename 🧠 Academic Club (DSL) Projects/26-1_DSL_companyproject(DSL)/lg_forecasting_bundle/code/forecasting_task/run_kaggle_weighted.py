...
def compute_consensus_clusters(df: pd.DataFrame, emb_dir: str, train_end: str, n_clusters: int):
    ...
    # OOM Fix: Load only gemini_textemb.parquet and linq_textemb.parquet for clustering
    parquet_files = [Path(emb_dir) / "gemini_textemb.parquet", Path(emb_dir) / "linq_textemb.parquet"]
...
class ConsensusDataset(Dataset):
    ...
    def __getitem__(self, i):
        return (torch.from_numpy(self.px[i]), torch.from_numpy(self.py[i]), torch.from_numpy(self.le[i]), 
...
def train_phase1(loader, text_dim, level_indices, args, device):
    ...
    for ep in range(1, args.text_epochs + 1):
        ...
        t_acc, cnt = 0.0, 0
        for batch in loader:
            px, le, xm, ym, dec_inp, gt = _prep_batch(batch, device, args.pred_len, args.label_len)
...
def run_epoch(loader, model, args, device, optimizer=None):
    ...
    with torch.enable_grad() if is_train else torch.no_grad():
        for batch in loader:
            ...
            # News weighting: weight = 1.0 + 2.0 * has_text (total weight = 3.0 for news samples)
            has_text = (le[:, model.text_clf.level_indices, :].abs().sum(dim=(1, 2)) > 1e-6).float().view(-1)
            news_weight = 1.0 + 2.0 * has_text
            
            if is_train:
                # Per-sample standard MSE loss
                sample_mse_loss = torch.mean((out - gt)**2, dim=(1, 2))
                
                # Per-sample Directional Penalty
                sample_dir_penalty = torch.clamp(torch.relu(-1.0 * pred_diff * true_diff), max=1.0)
                
                # Combine and apply News Weight
                sample_loss = sample_mse_loss + args.dir_weight * sample_dir_penalty
                loss = (sample_loss * news_weight).mean()
                
                if args.joint:
                    target = (gt[:, -1, 3] > px[:, -1, 3]).float()
                    sample_dir_loss = F.binary_cross_entropy(p_dir.view(-1), target, reduction='none')
                    # Apply aux_weight and add to overall loss
                    loss += args.aux_weight * (sample_dir_loss * has_text).mean()
                ...
                loss = mse + args.dir_weight * dir_penalty
            ...
            if not is_train:
                all_pred.append(pred_diff.detach().cpu().numpy())
                all_true.append(true_diff.detach().cpu().numpy())
                all_has_text.append(has_text.detach().cpu().numpy())
                
    if not is_train:
        p_arr = np.concatenate(all_pred)
        t_arr = np.concatenate(all_true)
        text_arr = np.concatenate(all_has_text)
        
        # Standard DA
        correct = (p_arr * t_arr > 0).astype(np.float32)
        da = correct.mean()
        
        # News-Weighted DA
        weights = 1.0 + 2.0 * text_arr
        weighted_da = (correct * weights).sum() / weights.sum()
        
        # RankIC
        from scipy.stats import spearmanr
        ic, _ = spearmanr(p_arr, t_arr)
        
        return {'mse': mse_sum / max(cnt, 1), 'da': da, 'weighted_da': weighted_da, 'rank_ic': ic if not np.isnan(ic) else 0.0}
    
    return {'mse': mse_sum / max(cnt, 1)}
...
def main():
    ...
    for ep in range(1, args.num_epoch + 1):
        tr = run_epoch(train_dl, model, args, device, opt)
        te = run_epoch(test_dl, model, args, device)
        print(f"  [Ep{ep:02d}] Train MSE: {tr['mse']:.5f} | Test MSE: {te['mse']:.5f} | Test DA: {te.get('da', 0):.4f} | Weighted DA: {te.get('weighted_da', 0):.4f} | Test RankIC: {te.get('rank_ic', 0):.4f}")
        
    # Save the best model (using the final epoch for simplicity, or we could track best)
    torch.save(model.state_dict(), '/root/LG-AI/best_kaggle_model.pth')
    print("\n[Done] Pipeline executed successfully. Model saved to best_kaggle_model.pth.")
