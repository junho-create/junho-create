#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""추출 결과와 정답 파일을 비교하는 스크립트.

사용법:
  python compare_results.py sample1
  python compare_results.py --all
  python compare_results.py --all --detailed
"""
import json
import argparse
    ...
    for field in sorted(all_fields):
        ...
        if field not in actual:
            ...
            details.append({
                ...
                "message": f"✗ Field missing in actual result"
            })
            continue
...
def main():
    parser = argparse.ArgumentParser(
        description="추출 결과와 정답 파일을 비교합니다.",
...
사용 예시:
  ...
  %(prog)s --all --detailed           # 상세 출력 포함
    ...
    if args.all:
        ...
        if not actual_files:
            print("[ERR] outputs/json/에 결과 파일이 없습니다.")
            return
        ...
        if not actual_path.exists():
            print(f"[ERR] 결과 파일이 없습니다: {actual_path}")
            return
        
        result = compare_result(actual_path, expected_path, args.detailed)
        print_comparison(result, args.detailed)
        
        if args.detailed and result["details"]:
            print("\n모든 필드 상세:")
            for detail in result["details"]:
                print(f"  {detail['field']}: {detail['message']}")
    else:
