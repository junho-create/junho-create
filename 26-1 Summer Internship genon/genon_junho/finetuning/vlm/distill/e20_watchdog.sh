#!/bin/bash
# e20 학습 감시 스크립트 (e19_watchdog.sh 기반, 경로만 e20 로 교체)
# - 학습이 (SIGTERM 등으로) 죽으면 최신 checkpoint 를 찾아 resume_from_adapter 로
#   이어붙여 자동 재시작한다.
# - 살아있는 동안에는 30초마다 확인만. 정상 완료(exit 0)면 감시도 종료.
#
# 실행:
#   cd /home/jhyeo/finetuning/vlm && source .venv/bin/activate
#   nohup bash distill/e20_watchdog.sh >/dev/null 2>&1 &
set -uo pipefail

VLM=/home/jhyeo/finetuning/vlm
cd "$VLM"

RUN_ROOT="output/e20_qwen35_9b_pixel_bboxw"
BASE_CONFIG="config/e20_qwen35_9b_pixel_bboxw.yaml"
GEN_DIR="config/_watchdog"
LOG_DIR="_run_logs"
mkdir -p "$GEN_DIR" "$LOG_DIR"
WLOG="$LOG_DIR/e20_watchdog.log"
...
t = open(path).read()
t = t.replace('output_dir: "./output/e20_qwen35_9b_pixel_bboxw/student_sft"',
              f'output_dir: "./output/e20_qwen35_9b_pixel_bboxw_resume_{ts}/student_sft"')
t = t.replace('run_name: "e20_qwen35_9b_pixel_bboxw"',
              f'run_name: "e20_qwen35_9b_pixel_bboxw_resume_{ts}"')
if ckpt:
...
PY
  ...
  CONFIG="$gen" \
  PHASE=sft \
  bash distill/run_distill.sh >> "$LOG_DIR/e20_watchdog_run_${ts}.log" 2>&1
  rc=$?
