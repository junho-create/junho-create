...
class TFLOPModelPLModule(pl.LightningModule):
    ...
    def _apply_finetune_freeze(self):
        """
        Finetuning 시 encoder / decoder 일부만 학습하도록 설정
        """

        # 1️⃣ Vision Encoder 전체 freeze
        if getattr(self.config, "freeze_encoder", False):
            for p in self.model.encoder.parameters():
                p.requires_grad = False

        # 2️⃣ Decoder 일부만 학습
        n = getattr(self.config, "finetune_last_n_decoder_layers", 0)
        ...
        if n > 0:
            ...
            # 마지막 n개 layer만 다시 활성화
            decoder_layers = list(self.model.decoder.layers)
