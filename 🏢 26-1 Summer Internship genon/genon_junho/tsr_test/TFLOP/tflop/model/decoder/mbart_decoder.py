...
class MBARTDecoder(nn.Module):
    ...
    def _custom_prepare_decoder_attention_mask(
        ...
        if input_shape[-1] > 1:
            ...
            if (
                ...
                combined_attention_mask = combined_attention_mask.clone() # shkim
    ...
    def get_img_roiAlign(self, encoder_hidden_states, quad_input_coords):
        ...
        feature_map = encoder_hidden_states.transpose(1, 2).contiguous().view(
        ...
        target_dtype = next(self.roi_proj.parameters()).dtype
        pooled_features = self.roi_proj(
            pooled_features.to(target_dtype)
        )  # (bsz * bbox_token_cnt, d_model)
    ...
    def forward(
        ...
        # shkim
        target_dtype = self.model.model.decoder.layers[0].encoder_attn.k_proj.weight.dtype
